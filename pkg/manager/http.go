...
func (serv *HTTPServer) httpMain(w http.ResponseWriter, r *http.Request) {
	...
	if len(serv.crashStores()) != 0 {
		var err error
		data.Crashes, data.AvailableSubsystems, err = serv.collectCrashes(filterSubsystems)
		if err != nil {
...
func (serv *HTTPServer) collectCrashes(filterSubsystems []string) ([]UICrashType, []string, error) {
	repros := serv.reproducing()
	var ret []UICrashType
	var allInfos []*BugInfo
	...
	if len(filterSubsystems) != 0 {
		available = availableSubsystems(allSubsystems(allInfos), filterSubsystems)
	}
	return ret, available, nil
}
