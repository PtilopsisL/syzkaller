...
func TestKernelRuntimeMachineCheckedUsesProvidedSource(t *testing.T) {
	...
	next, err := runtime.MachineChecked(0, testKernelRuntimeSyscalls(cfg.Target))
	require.NoError(t, err)
...
func TestKernelRuntimeTriageProgress(t *testing.T) {
	...
	_, err = runtime.MachineChecked(0, testKernelRuntimeSyscalls(cfg.Target))
	require.NoError(t, err)
...
func (inst *testKernelRuntimeInstance) Run(context.Context, string) (<-chan []byte, <-chan error, error) {
	outc := make(chan []byte)
